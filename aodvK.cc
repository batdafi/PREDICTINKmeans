...
AODVExample::PrintArguments()
{
	...
	std::cout << "Region area: " << areaX << " X " << areaY << "\n";
...
AODVExample::Run()
{
    ...
    std::ofstream positionPrint("KMEANS-node-positions.txt");
    positionPrint.close();
    Simulator::Schedule(Seconds(1.0), &AODVExample::PrintPositions, this);
...
//batdafi add
    std::stringstream mamamx;
    mamamx << "ns3::UniformRandomVariable[Min=0.0|Max=" << areaX << "]";
    std::stringstream mamamy;
    mamamy << "ns3::UniformRandomVariable[Min=0.0|Max=" << areaY << "]";
	
    pos.Set("X", StringValue(mamamx.str()));
    pos.Set("Y", StringValue(mamamy.str()));
//done
